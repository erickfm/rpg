...
| LANDED | A | diner blade illegible | GOTCHAS §10 CHECKED FIRST AND RULED OUT: the blade is a `BoxGeometry`, not a DoubleSide plane, so every face carries its own correctly-oriented UVs — verified by walking past and reading it from BOTH directions, not by asserting it. The real cause is arithmetic: the plate is `masonry(0.95, 1.55)` at 16 px/m = **15 x 25 texels**, the border takes 2 rows top and bottom, and three 8 px letters at 5 px centres overlapped by ~3 px each. Three stacked letters do not fit at any size, so it is a **coffee cup** — a symbol reads where letters cannot and the fascia already says DINER. Also fixed a defect nobody reported, in the same sign: it never went dark, because my enamel cream (232 max, chroma 28) cleared props.ts's bright-and-chromatic light-source test by two points. Night luminance 152 -> 55. Commits 382ef4f8d, 10820391f |

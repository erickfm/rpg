...
//
// `drop` is the mutation hook for --selftest: the names of meshes to pretend do
// not exist. A raycaster that cannot be made to report VOID is a raycaster that
// reports FLOOR unconditionally, and that is a check that can never fail.
const sweep = await page.evaluate(([GRID, FLOOR_LO, FLOOR_HI, drop]) => {

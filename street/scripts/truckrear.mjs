...
for(const [n,lx,lz] of [['rearq',2.6,4.0],['flank',3.0,1.6]]){
 const [ex,ez]=L2W(lx,lz);
 const gy=await p.evaluate(([x,z])=>window.__ct.groundAt(x,z),[ex,ez]);
 const [bx,bz]=L2W(0,1.6);
 await p.evaluate(([x,z,y,g])=>window.__ct.warp(x,z,y,g,-0.10),[ex,ez,Math.atan2(bx-ex,-(bz-ez)),gy]);
 await afterFrames(p,4); const got=await p.evaluate(()=>window.__ct.pos());
 await p.screenshot({path:`shots/tr-${n}.png`});
 console.log(`  tr-${n}.png from (${got[0].toFixed(1)}, ${got[2].toFixed(1)}) ${Math.hypot(got[0]-ex,got[2]-ez)<0.9?'landed':'** MISSED'}`);
}
await b.close();

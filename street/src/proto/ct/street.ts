...
export function buildStreet(o: {
  ...
  // ── the alley: a dark cut in the left wall with a dumpster ──────────────
  {
    ...
    const CAT_DESIGNS: { nm: string; draw: (p: CatPx, g: CanvasRenderingContext2D) => void }[] = [
      // 1 — ginger tabby, upright sit, tail curled round the front paws
      {
        nm: 'ginger', draw: (p, g) => {
          const F = '#a86a3c', S = '#8a5228', C = '#d9c2a0', P = '#b5766e';
          const LI = 'rgba(255,255,255,0.18)', SH = 'rgba(0,0,0,0.18)';
          p(F, [4, 0, 2, 1], [3, 1, 4, 1], [3, 2, 5, 1], [3, 3, 5, 1], [14, 0, 2, 1], [13, 1, 4, 1], [12, 2, 5, 1], [12, 3, 5, 1]);
          p(P, [4, 2, 3, 1], [4, 3, 3, 1], [13, 2, 3, 1], [13, 3, 3, 1]);
          p(F, [3, 4, 14, 10]);
          g.clearRect(3, 13, 1, 1); g.clearRect(16, 13, 1, 1);
          p(LI, [3, 4, 2, 9]); p(SH, [15, 4, 2, 9]);
          p(S, [6, 4, 1, 2], [9, 4, 2, 2], [13, 4, 1, 2], [3, 10, 2, 1], [15, 10, 2, 1]);
          p('#7d9463', [5, 8, 4, 4], [11, 8, 4, 4]);
          p('#241a12', [6, 9, 2, 3], [12, 9, 2, 3]);
          p('#f2ead0', [6, 9, 1, 1], [12, 9, 1, 1]);
          p(P, [9, 12, 2, 1]); p('#241a12', [8, 13, 1, 1], [11, 13, 1, 1]);
          p('#efe8d8', [0, 10, 3, 1], [0, 12, 3, 1], [17, 10, 3, 1], [17, 12, 3, 1]);
          for (const [x, y, w, h] of [[6, 14, 8, 3], [5, 17, 10, 4], [4, 21, 11, 4]] as Block[]) {
            p(F, [x, y, w, h]); p(LI, [x, y, 2, h]); p(SH, [x + w - 2, y, 2, h]);
          }
          p(S, [5, 19, 2, 1], [12, 19, 2, 1], [4, 23, 2, 1], [12, 23, 2, 1]);
          p(C, [8, 15, 4, 7]); p('rgba(0,0,0,0.2)', [8, 22, 4, 2]);
          p(F, [6, 20, 3, 5], [10, 20, 3, 5]);
          p(C, [6, 24, 3, 3], [10, 24, 3, 3]);
          p(S, [6, 26, 1, 1], [12, 26, 1, 1]);
          p(F, [14, 18, 2, 1], [17, 19, 2, 5], [14, 25, 5, 2], [3, 26, 12, 2], [1, 23, 2, 4]);
          p(S, [17, 21, 2, 1], [15, 25, 1, 2], [10, 26, 1, 2], [6, 26, 1, 2]);
          p(C, [1, 23, 2, 2]);
        },
      },
      // 2 — grey tabby in a loaf: no legs, head sunk onto the shoulders
      {
        nm: 'grey-loaf', draw: (p, g) => {
          const F = '#7c7a76', S = '#5a5854', C = '#cfcac0', P = '#a8807a';
          const LI = 'rgba(255,255,255,0.18)', SH = 'rgba(0,0,0,0.18)';
          p(F, [2, 20, 15, 6], [3, 26, 13, 2]);
          g.clearRect(2, 20, 1, 1); g.clearRect(16, 20, 1, 1);
          g.clearRect(3, 27, 1, 1); g.clearRect(15, 27, 1, 1);
          p(LI, [2, 20, 2, 6]); p(SH, [15, 20, 2, 6]);
          p(S, [3, 22, 4, 1], [12, 22, 4, 1], [4, 25, 3, 1], [12, 25, 3, 1]);
          p(F, [4, 10, 2, 1], [3, 11, 4, 1], [3, 12, 5, 1], [14, 10, 2, 1], [13, 11, 4, 1], [12, 12, 5, 1]);
          p(P, [4, 12, 3, 1], [13, 12, 3, 1]);
          p(F, [3, 13, 14, 9]);
          g.clearRect(3, 21, 1, 1); g.clearRect(16, 21, 1, 1);
          p(LI, [3, 13, 2, 8]); p(SH, [15, 13, 2, 8]);
          p(S, [6, 13, 1, 2], [9, 13, 2, 2], [13, 13, 1, 2]);
          p('#b09040', [5, 16, 4, 4], [11, 16, 4, 4]);
          p('#241a12', [6, 17, 2, 3], [12, 17, 2, 3]);
          p('#f2ead0', [6, 17, 1, 1], [12, 17, 1, 1]);
          p(P, [9, 20, 2, 1]); p('#241a12', [8, 21, 1, 1], [11, 21, 1, 1]);
          p('#efe8d8', [0, 18, 3, 1], [0, 20, 3, 1], [17, 18, 3, 1], [17, 20, 3, 1]);
          p(C, [5, 26, 3, 2], [11, 26, 3, 2]);
          p(F, [16, 18, 2, 2], [18, 20, 2, 5], [14, 25, 5, 2]);
          p(S, [18, 22, 2, 1], [15, 25, 1, 2]);
        },
      },
      // 3 — black cat, alert, tail held straight out. Nothing but silhouette.
      {
        nm: 'black', draw: (p, g) => {
          const F = '#2f2d33', PAW = '#4a4850', P = '#8a5f62';
          const LI = 'rgba(255,255,255,0.22)', SH = 'rgba(0,0,0,0.25)';
          p(F, [3, 0, 3, 1], [2, 1, 5, 1], [2, 2, 6, 1], [2, 3, 6, 1], [14, 0, 3, 1], [13, 1, 5, 1], [12, 2, 6, 1], [12, 3, 6, 1]);
          p(P, [3, 2, 3, 1], [3, 3, 3, 1], [13, 2, 3, 1], [13, 3, 3, 1]);
          p(F, [2, 4, 16, 10]);
          g.clearRect(2, 13, 1, 1); g.clearRect(17, 13, 1, 1);
          p(LI, [2, 4, 2, 9]); p(SH, [16, 4, 2, 9]);
          p('#d0a83c', [4, 8, 4, 4], [12, 8, 4, 4]);
          p('#141216', [5, 9, 2, 3], [13, 9, 2, 3]);
          p('#f2ead0', [5, 9, 1, 1], [13, 9, 1, 1]);
          p(P, [9, 12, 2, 1]); p('#141216', [8, 13, 1, 1], [11, 13, 1, 1]);
          p('#efe8d8', [0, 10, 2, 1], [0, 12, 2, 1], [18, 10, 2, 1], [18, 12, 2, 1]);
          for (const [x, y, w, h] of [[7, 14, 6, 3], [6, 17, 8, 4], [5, 21, 9, 5]] as Block[]) {
            p(F, [x, y, w, h]); p(LI, [x, y, 2, h]); p(SH, [x + w - 2, y, 2, h]);
          }
          p(PAW, [6, 24, 3, 3], [10, 24, 3, 3]);
          p(F, [13, 21, 7, 2]);
          p(LI, [13, 22, 7, 1]);
        },
      },
      // 4 — tuxedo: white shirt front, white socks, white tail tip
      {
        nm: 'tuxedo', draw: (p, g) => {
          const F = '#3a3a40', W = '#ded9cf', P = '#b58a86';
          const LI = 'rgba(255,255,255,0.18)', SH = 'rgba(0,0,0,0.18)';
          p(F, [4, 0, 2, 1], [3, 1, 4, 1], [3, 2, 5, 1], [3, 3, 5, 1], [14, 0, 2, 1], [13, 1, 4, 1], [12, 2, 5, 1], [12, 3, 5, 1]);
          p(P, [4, 2, 3, 1], [13, 2, 3, 1]);
          p(F, [3, 4, 14, 10]);
          g.clearRect(3, 13, 1, 1); g.clearRect(16, 13, 1, 1);
          p(LI, [3, 4, 2, 9]); p(SH, [15, 4, 2, 9]);
          p(W, [8, 4, 4, 5], [6, 11, 8, 3]);
          p('#6f8f9e', [5, 8, 4, 3], [11, 8, 4, 3]);
          p('#241a12', [6, 9, 2, 2], [12, 9, 2, 2]);
          p('#f2ead0', [6, 9, 1, 1], [12, 9, 1, 1]);
          p(P, [9, 11, 2, 1]); p('#241a12', [8, 12, 1, 1], [11, 12, 1, 1]);
          p('#efe8d8', [0, 10, 3, 1], [0, 12, 3, 1], [17, 10, 3, 1], [17, 12, 3, 1]);
          for (const [x, y, w, h] of [[6, 14, 8, 3], [5, 17, 10, 4], [4, 21, 11, 4]] as Block[]) {
            p(F, [x, y, w, h]); p(LI, [x, y, 2, h]); p(SH, [x + w - 2, y, 2, h]);
          }
          p(W, [7, 14, 6, 9]);
          p(F, [6, 20, 3, 4], [10, 20, 3, 4]);
          p(W, [6, 23, 3, 4], [10, 23, 3, 4]);
          p(F, [14, 18, 2, 1], [17, 19, 2, 5], [14, 25, 5, 2], [3, 26, 12, 2], [1, 23, 2, 4]);
          p(W, [1, 23, 2, 2]);
        },
      },
      // 5 — calico curled up, head down on the near end, tail all the way round
      {
        nm: 'calico-curl', draw: (p, g) => {
          const W = '#d8d2c4', O = '#b8763a', K = '#3a3230', P = '#b5766e';
          const LI = 'rgba(255,255,255,0.16)', SH = 'rgba(0,0,0,0.18)';
          p(W, [5, 14, 12, 3], [3, 17, 14, 6], [5, 23, 11, 3], [7, 26, 8, 2]);
          g.clearRect(5, 14, 1, 1); g.clearRect(16, 14, 1, 1);
          p(LI, [3, 17, 2, 6]); p(SH, [15, 17, 2, 6]);
          p(O, [10, 15, 5, 4], [6, 21, 4, 3]);
          p(K, [12, 20, 4, 4], [6, 17, 4, 3]);
          p(W, [2, 6, 2, 1], [1, 7, 4, 1], [1, 8, 5, 1], [10, 6, 2, 1], [9, 7, 4, 1], [8, 8, 5, 1]);
          p(P, [2, 8, 2, 1], [9, 8, 2, 1]);
          p(W, [1, 9, 12, 8]);
          g.clearRect(1, 16, 1, 1); g.clearRect(12, 16, 1, 1);
          p(LI, [1, 9, 2, 7]); p(SH, [11, 9, 2, 7]);
          p(O, [1, 9, 4, 3]); p(K, [9, 9, 4, 3]);
          p('#8a9a62', [3, 12, 3, 3], [8, 12, 3, 3]);
          p('#241a12', [4, 13, 1, 2], [9, 13, 1, 2]);
          p('#f2ead0', [3, 12, 1, 1], [8, 12, 1, 1]);
          p(P, [6, 15, 2, 1]); p('#241a12', [5, 16, 1, 1], [8, 16, 1, 1]);
          p('#efe8d8', [0, 13, 1, 1], [0, 15, 1, 1], [13, 13, 3, 1], [13, 15, 3, 1]);
          p(O, [18, 19, 2, 5], [14, 24, 6, 2], [2, 26, 13, 2]);
          p(K, [18, 21, 2, 1], [9, 26, 1, 2], [5, 26, 1, 2]);
        },
      },
      // 6 — brown tabby, head cocked, one paw up, long tail out behind
      {
        nm: 'brown-tabby', draw: (p, g) => {
          const F = '#6e5236', S = '#48331f', C = '#c9b895', P = '#a8776d';
          const LI = 'rgba(255,255,255,0.18)', SH = 'rgba(0,0,0,0.18)';
          p(F, [5, 0, 2, 1], [4, 1, 4, 1], [4, 2, 5, 1], [4, 3, 5, 1], [15, 1, 2, 1], [14, 2, 4, 1], [13, 3, 5, 1]);
          p(P, [5, 2, 3, 1], [5, 3, 3, 1], [14, 3, 3, 1]);
          p(F, [4, 4, 14, 10]);
          g.clearRect(4, 13, 1, 1); g.clearRect(17, 13, 1, 1);
          p(LI, [4, 4, 2, 9]); p(SH, [16, 4, 2, 9]);
          p(S, [7, 4, 1, 3], [10, 4, 2, 3], [14, 4, 1, 3], [4, 9, 2, 1], [16, 9, 2, 1], [4, 11, 2, 1], [16, 11, 2, 1]);
          p('#9aa860', [6, 8, 4, 4], [12, 8, 4, 4]);
          p('#241a12', [7, 9, 2, 3], [13, 9, 2, 3]);
          p('#f2ead0', [7, 9, 1, 1], [13, 9, 1, 1]);
          p(P, [10, 12, 2, 1]); p('#241a12', [9, 13, 1, 1], [12, 13, 1, 1]);
          p('#efe8d8', [1, 10, 3, 1], [1, 12, 3, 1], [18, 10, 2, 1], [18, 12, 2, 1]);
          for (const [x, y, w, h] of [[7, 14, 8, 3], [6, 17, 10, 4], [6, 21, 11, 5]] as Block[]) {
            p(F, [x, y, w, h]); p(LI, [x, y, 2, h]); p(SH, [x + w - 2, y, 2, h]);
          }
          p(S, [7, 19, 3, 1], [13, 19, 3, 1], [6, 23, 3, 1], [14, 23, 3, 1]);
          p(C, [9, 15, 4, 7]); p('rgba(0,0,0,0.2)', [9, 22, 4, 2]);
          p(F, [8, 20, 3, 4], [13, 20, 3, 4]);
          p(C, [8, 24, 3, 3], [13, 24, 3, 3]);
          p(F, [5, 18, 2, 2]); p(C, [3, 17, 3, 3]);
          p(F, [0, 21, 5, 2]);
          p(S, [1, 21, 1, 2], [3, 21, 1, 2]);
          p(C, [0, 21, 1, 2]);
        },
      },
    ];
